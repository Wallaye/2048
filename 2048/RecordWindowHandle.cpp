...
void RecordWindowHandle::ReadRecords() {
	...
	std::ifstream file(L"..\\Records.txt");
	std::string buffer;
...
void RecordWindowHandle::SaveRecords()
{
	...
	std::ofstream file(L"..\\Records.txt");
	std::wstring buffer;
...
LRESULT RecordWindowHandle::RecordWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		_worker->BeginScene();
		_worker->FillRect(RectF(0, 0, 350, 600), Color(240, 240, 240));
		for (std::map<int, std::wstring>::reverse_iterator it = records.rbegin(); it != records.rend(); ++it) {
			WCHAR buff[64];
			lstrcpyW(buff, std::to_wstring(i + 1).c_str());
			lstrcatW(buff, L". ");
			lstrcatW(buff, it->second.c_str());
			lstrcatW(buff, L" ");
